...
Same as before, this time we also used a F* MCP that Lef built.

## The Good, The Bad, and the Ugly: A Brief Tour of our Experience

### DLL

Highlight cyclic structures.
Point to prior pain with DLL in Low*
Separation logic is important and helps

### RB Trees

Models know a lot already! This is great, but also maybe a hindrance if the goal
is to formalize a given text. Models sometimes don't really seem to read the
text at all, and just go for it from background knowledge.

So, RBTrees: first attempt was good, but it was actually implementing
Okasaki-style trees.

With critique from Gabriel's review, and more nudging, we got it to implement
the parent-pointer-based one with rotations etc.

### Highlight on Dijsktra's algorithm
